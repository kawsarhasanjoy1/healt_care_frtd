...
import { useGetMyReviewQuery } from "@/app/redux/api/reviewApi";
...
const ReviewsPage = () => {
  ...
  const { data, isLoading } = useGetMyReviewQuery({ ...filters });
  ...
  const column = [
      ...
      render: (row: any) => (
        <div className="flex items-center gap-2">
          ...
          <div className=" flex flex-col space-y-1.5">
            ...
            <span>{row?.doctor?.email}</span>
            <span>{row?.doctor?.registrationNumber}</span>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (row: any) => (
        <p
          className="max-w-[250px] truncate text-sm text-slate-600"
          title={row.comment}
        >
          {row.comment}
        </p>
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Monitor and manage doctor reviews and ratings.
          </p>
        </div>
      </div>
